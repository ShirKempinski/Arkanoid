...
public class Ball implements Sprite {
     ...
     */
    public Ball(int x, int y, int r, java.awt.Color color, Velocity velocity, GameEnvironment game) {
        this.center = new Point((double) x, (double) y);
        if (r < 0) {
            this.size = -r;
        } else {
    ...
    public void moveOneStep() {
        CollisionInfo info = this.environment.getClosestCollision(getTrajectory());
        if (info == null) {
